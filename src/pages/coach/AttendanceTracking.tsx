...
const AttendanceTracking: React.FC = () => {
  const { user, students, schedules, attendances, markAttendance, darkMode } = useAppContext();
  ...
  const exportAttendanceToCSV = () => {
    ...
    link.setAttribute('download', `asistencia_${new Date(selectedDate).toLocaleDateString('es-ES').replace(/\//g, '-')}.csv`);
    ...
    // Show success alert
    alert(`¡Exportación exitosa!\n\nEl archivo de asistencia del ${new Date(selectedDate).toLocaleDateString('es-ES')} se ha descargado correctamente.`);
  };
